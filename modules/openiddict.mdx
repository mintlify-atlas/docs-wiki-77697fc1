...
`AbpOpenIddictDomainModule` registers a background worker that periodically prunes expired tokens and authorizations from the store. The isolation levels used during prune and delete operations are configurable via `AbpOpenIddictStoreOptions`:

```csharp
// modules/openiddict/src/.../AbpOpenIddictStoreOptions.cs
Configure<AbpOpenIddictStoreOptions>(options =>
{
    // Default: IsolationLevel.RepeatableRead
