...
├── Volo.Abp.Identity.Blazor.MudBlazor               # MudBlazor components (shared)
├── Volo.Abp.Identity.Blazor.MudBlazor.Server        # MudBlazor Server variant
├── Volo.Abp.Identity.Blazor.MudBlazor.WebAssembly   # MudBlazor WASM variant
├── Volo.Abp.Identity.AspNetCore                     # SignIn manager, security stamp
├── Volo.Abp.Identity.Installer                      # ABP CLI installer metadata
└── Volo.Abp.PermissionManagement.Domain.Identity    # Permission seeding for Identity
```
...
`IdentityRole` is an `AggregateRoot<Guid>` (also implements `IMultiTenant`, `IHasEntityVersion`, and `IHasCreationTime`) with `Name`, `NormalizedName`, `IsDefault`, and `IsPublic` properties. Roles marked `IsDefault` are automatically assigned to new users.
...
<Note>
`IIdentityUserLookupAppService` is marked `[Obsolete]`. Use `IIdentityUserIntegrationService` for module-to-module or service-to-service user lookups instead.
</Note>

Lightweight interface originally used for populating user-picker UI components:
...
`IdentityDataSeedContributor` creates an initial `admin` user and `admin` role when the database is seeded. It reads seed values from `DataSeedContext` using the property-name constants defined on the contributor itself. Override them in tests or CI by populating the context before seeding:

```csharp
// IdentityDataSeedContributor defines these constants:
// AdminEmailPropertyName    = "AdminEmail"      default: "admin@abp.io"
// AdminUserNamePropertyName = "AdminUserName"   default: "admin"
// AdminPasswordPropertyName = "AdminPassword"   default: "1q2w3E*"

var context = new DataSeedContext();
context[IdentityDataSeedContributor.AdminEmailPropertyName]    = "admin@example.com";
context[IdentityDataSeedContributor.AdminPasswordPropertyName] = "MyStr0ng!Pass";
