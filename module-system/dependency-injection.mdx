    ...
    public override void AddType(IServiceCollection services, Type type)
    {
        ...
        foreach (var exposedServiceType in exposedServiceAndKeyedServiceTypes)
        {
            var serviceDescriptor = CreateServiceDescriptor(
                ...
                exposedServiceAndKeyedServiceTypes, lifeTime.Value);
...
public static class ExposedServiceExplorer
{
    ...
    public static List<Type> GetExposedServices(Type type)
    {
        ...
        // If there are only keyed-service providers and no regular providers,
        // do not expose default services.
...
- **Property injection** — `AbpPropertySelector` injects all public settable properties whose type is registered in the container, unless the property is marked with `[DisablePropertyInjection]`
...
`UseAutofac()` creates a `ContainerBuilder`, registers it as an object accessor in the service collection, and installs `AbpAutofacServiceProviderFactory`. You can access the raw `ContainerBuilder` via:
