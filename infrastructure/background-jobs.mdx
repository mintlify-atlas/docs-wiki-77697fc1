...
protected override async Task DoWorkAsync(PeriodicBackgroundWorkerContext workerContext)
{
    ...
    {
        if (handle == null) return; // another node is processing
...
<Tabs>
  ...
  <Tab title="TickerQ (Recurring)">
    `Volo.Abp.TickerQ` adds first-class recurring-job support with cron expressions via `ITickerQManager`.

    ```bash
    dotnet add package Volo.Abp.TickerQ
    ```

    ```csharp
    [DependsOn(typeof(AbpTickerQModule))]
    public class MyModule : AbpModule { }

    // Define a recurring job
    public class DailyReportJob : ITickerQJob, ITransientDependency
    {
        public async Task ExecuteAsync(TickerQJobExecutionContext context)
        {
            // generate report
        }
    }
