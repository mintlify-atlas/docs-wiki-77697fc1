...
`IsGrantedAsync` accumulates grants — once any provider returns `Granted` the flag is set, but the loop continues so that a later `Prohibited` can still override. `Undefined` means the provider abstains and the chain continues.
