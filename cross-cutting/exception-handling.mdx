...
{
    options.SendExceptionsDetailsToClients = false; // hide details in production
    options.SendStackTraceToClients        = false; // default is true; explicitly disable in production
});
```

<Warning>
`SendExceptionsDetailsToClients` and `SendStackTraceToClients` should both be `false` in production. `SendStackTraceToClients` defaults to `true`, so explicitly set it to `false` before going live. Stack traces expose internal implementation details that can aid attackers.
</Warning>
