...
`AbpDbContext<TDbContext>` extends `Microsoft.EntityFrameworkCore.DbContext` and implements `IAbpEfCoreDbContext` and `IAbpEfCoreDbFunctionContext`. Every ABP application database context inherits from it:
...
The evaluated values (`IsSoftDeleteFilterEnabled`, `IsMultiTenantFilterEnabled`, `CurrentTenantId`) are instance properties, so EF Core re-evaluates them on every query — enabling per-request filter toggling via `IDataFilter`.
