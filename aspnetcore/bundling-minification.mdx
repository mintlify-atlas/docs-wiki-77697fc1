...
- **Environment-aware file serving**: in `Development` mode, each file is served individually; in production, a single hashed bundle URL is emitted.
...
public class MyModuleScriptContributor : BundleContributor
{
    ...
    public override Task ConfigureDynamicResourcesAsync(
        BundleConfigurationContext context)
    {
        // Emit inline dynamic content (e.g., localisation JSON)
        context.DynamicResources.Add(new MyDynamicScriptResource());
...
`AbpBundlingOptions` is the central configuration object. It lives in `Volo.Abp.AspNetCore.Bundling`:
...
public class AbpBundlingOptions
{
    ...
    public bool? IsMinificationEnabled { get; set; }
    // null → read from environment (true in Production)
}
```

<Tip>
Override minification explicitly when debugging a production deployment:
```csharp
Configure<AbpBundlingOptions>(options =>
{
    options.IsMinificationEnabled = false;
});
```
</Tip>
...
<Note>
CDN integration: configure `AbpBundlingOptions.BundleCdnUrl` to prepend a CDN base URL to all bundle `<src>` attributes, enabling edge-caching of static assets.
</Note>
